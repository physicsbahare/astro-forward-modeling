...
_RUNNER = Path(__file__).resolve().parents[1] / "scripts" / "run_pyautogalaxy_morphology_preflight.py"
_SPEC = importlib.util.spec_from_file_location("b9a_run_pyautogalaxy_morphology_preflight", _RUNNER)
...
def test_pinned_versions_and_frozen_scope():
    ...
    c = exp.config()
    assert c["over_sample_size"] == 1
    assert "no morphology band" in c["acceptance"]
