...
- [x] **Passive Spiral P1 spiral-feature survival under controlled artificial redshifting.** Run `34025358730` passed all three regression tests. With no added noise/evolution and no sharpening, the known matched arm amplitude is suppressed by about 9–16% across z=0.5–3 while the artificial-redshift path reproduces the same-scene direct-target control to about `3.2–3.4e-4` in arm-amplitude ratio error and the smooth control remains at numerical-zero matched-arm power. This is a resolution/operator diagnostic, not a spiral-classification threshold or independent cross-code validation; see `benchmarks/passive_spiral_redshift/P1_RESULT.md`.
...
**Status: active. Real COSMOS-Web L1 injection/recovery is established; D1o shows that literal source-shot covariance is not identifiable from the compact SCI/ERR/WHT bundle; D2b establishes authoritative release/product context but not full replay; and D2c now establishes exact release membership at the single frozen anchor pixel from the full A1 mosaic itself. Literal exposure-space source-shot realization remains blocked because pre-resample calibration/count/variance provenance and the complete historical resampling operator are not yet closed.**
...
- [x] **D1n PSF/effective-scene and residual-causality diagnostic series.** Detector-field STPSF variation is modest; published COSMOS-Web empirical PSFs are materially broader than the declared STPSF, but a paired-difference wrong-PSF oracle shows that mismatch of the observed magnitude alone does not cause the ~1 mag catastrophes. Multistart/truth initialization does not remove them. Residual-dose restoration drives the two failures continuously, and after planar-background removal the catastrophic positions contain about 1.77–1.95% target-tangent residual power versus about 0.33–0.78% in controls at similar Jacobian conditioning. D1n-j then shows that the tangent component alone already causes substantial failure-specific bias (`delta_mag` about -0.60/-0.41; `Re` about 0.28 arcsec from a 0.18-arcsec truth), while the orthogonal component alone is not independently catastrophic for the isolated case and the combined residual amplifies the final collapse nonlinearly. One near-source orthogonal-only fit hits the lower Sérsic-index bound and is preserved. See `benchmarks/gate_d/COSMOSWEB_D1_RESIDUAL_TANGENT_SPLIT_RESULT.md`.
- [x] **D1o L1 source-shot-noise/covariance identifiability preflight.** Run `34017575982` executed successfully as a non-stochastic audit of the immutable real cutout. The artifact contains only `SCI/ERR/WHT`; `VAR_POISSON`, `VAR_RNOISE`, and `VAR_FLAT` are absent, as are an exposure-level source representation and replayable per-exposure resampling provenance. Therefore a physically exact source-Poisson realization plus drizzle covariance is not identifiable from this frozen compact L1 bundle. No noise or covariance correction was added. Independent output-pixel Poisson noise, if ever tested, must be labeled an L1 approximation rather than truth; see `benchmarks/gate_d/COSMOSWEB_D1_SOURCE_SHOT_COVARIANCE_PREFLIGHT_RESULT.md`.
- [x] **D2a exposure-level archive/provenance feasibility preflight.** Run `34019270052` completed successfully as software with five unit tests passing. A metadata-only MAST query at the frozen Gate-D anchor found eight distinct public F444W `_cal` candidate exposure roots from program 1727, but archive proximity does not establish literal COSMOS-Web DR1 contributor membership. Exact release ASN membership, literal pre-resample inputs, the actual contributing-cal variance/count inventory, and the exact historical resampling configuration remain unproven, so `source_shot_realization_permitted=false`. No science pixels or stochastic noise were generated and SCI/ERR/WHT were not modified; see `benchmarks/gate_d/COSMOSWEB_D2_EXPOSURE_PROVENANCE_PREFLIGHT_RESULT.md`.
- [x] **D2b authoritative release-membership/resampling-provenance audit.** Release documentation establishes tile A1, pipeline/CRDS context, final product structure/variance planes, scale and tangent point, but not literal contributors or complete historical resampling settings; source-shot realization remains blocked. See `benchmarks/gate_d/COSMOSWEB_D2B_RELEASE_PROVENANCE_RESULT.md`.
- [x] **D2c local full-mosaic pixel-level provenance audit.** The authoritative A1 `CON`+`HDRTAB` metadata decodes the single frozen anchor pixel to exactly four release inputs: OBS 084 NRCALONG exposures 1–4 (`*_jhat_a3001_crf.fits`). This closes exact release membership at that pixel only, not the whole cutout/tile, and does not equate those survey-processed files with archive `_cal` products. `source_shot_realization_permitted=false`; see `benchmarks/gate_d/COSMOSWEB_D2C_LOCAL_MOSAIC_PIXEL_PROVENANCE_RESULT.md`.
- [ ] **D2d release-recorded resampling metadata audit.** Read `RESWHT`, `PXSCLRT`, `PIXFRAC`, `NDRIZ`, contributor `R_DRZPAR`/`R_RESAMP`, and structural ASDF metadata from the same full product. Accept only explicitly recorded historical values; do not fill missing kernel/weight/operator settings from JWST defaults or from the outlier-rejection configuration. Then decide which replay requirements remain genuinely open; see `benchmarks/gate_d/COSMOSWEB_D2D_RESAMPLE_METADATA_PROTOCOL.md`.
