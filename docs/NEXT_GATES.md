...
**Status: active. Real COSMOS-Web L1 injection/recovery is established; the D1n series isolates a residual-alignment/nonlinear-amplification failure mechanism, and D1o shows that physically exact source-shot noise is not identifiable from the frozen L1 SCI/ERR/WHT bundle. The next non-redundant task is an L2 exposure-injection/provenance preflight.**
...
- [ ] **Design and execute a minimal L2 exposure-injection/provenance preflight.** Establish the exact contributing exposure products, calibration/unit boundary, WCS/pixel convention, variance provenance, and maintained JWST resampling settings on a small controlled subset before injecting stochastic source counts. Only after that preflight should L1-versus-L2 morphology/noise differences be measured.
