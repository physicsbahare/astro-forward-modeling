...
**Status: active. Real COSMOS-Web L1 injection/recovery is established; the D1n series isolates a residual-alignment/nonlinear-amplification failure mechanism; D1o proves that literal source-shot covariance is not identifiable from the frozen L1 SCI/ERR/WHT bundle; and D2a finds plausible public `_cal` exposures but does not establish their literal release membership/resampling provenance. The next non-redundant task is an authoritative release-membership/resampling-provenance audit before any stochastic exposure-space injection.**
...
- [ ] **D2b authoritative release-membership/resampling-provenance audit.** Locate release-authoritative COSMOS-Web association/reduction metadata for the exact Gate-D tile/region and determine whether the actual contributing exposure set, calibration/count boundary, variance inventory, and release resampling operator can be replayed without inference. Do not design or execute stochastic source-shot injection unless this closes the four D2a provenance gaps.
