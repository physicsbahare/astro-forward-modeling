...
- [ ] AGN nuclear-fraction morphology contamination benchmark — Stage-3c run `33680659156` succeeded; all 54 winners/162 starts reviewed, with nine bound-hit winners per arm. Stage-3d `gate-c-agn-estimated-weights` tests one model-estimated variance update against archived oracle fits of the same shot-noise images. Broader ensembles and PSF mismatch remain open. See `benchmarks/agn_nuclear_fraction/REVIEW.md`.
