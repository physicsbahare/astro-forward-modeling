...
- [ ] AGN nuclear-fraction morphology contamination benchmark — Stage-3a run `33668364723` succeeded; all 54 winners/162 starts reviewed, with 16 bound-hit winners despite optimizer success. Stage-3b `gate-c-agn-n-profile` profiles fixed n using archived n=4, ratio=1 images to distinguish weak constraints from missed optima. Source-shot noise and PSF mismatch remain separate/open. See `benchmarks/agn_nuclear_fraction/REVIEW.md`.
