...
- [ ] AGN nuclear-fraction morphology contamination benchmark — Stage-1 run `33642676932` confirmed successful and artifacts reviewed: all six decomposition winners recover truth, all six host-only winners hit bounds. Stage-2a `gate-c-agn-sampling` tests 4/8/16x sampling and fixed-shape flux sensitivity before noise. Sampling convergence and full identifiability remain unproven. Noise and PSF mismatch remain separate. See `benchmarks/agn_nuclear_fraction/REVIEW.md`.
