...
- [ ] AGN nuclear-fraction morphology contamination benchmark — Stage-3d run `33686694903` succeeded; all 54 winners/162 starts reviewed. Estimated-weight radii differ from oracle by at most 0.00457 of true Re, while nine final bound hits remain. Stage-3e `gate-c-agn-noise-ensemble` expands to 12 new paired seeds per case using the fixed two-pass estimator. Review all shards before gate classification or PSF-mismatch work. See `benchmarks/agn_nuclear_fraction/REVIEW.md`.
