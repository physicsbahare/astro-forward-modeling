...
- [ ] AGN nuclear-fraction morphology contamination benchmark — Stage-3b run `33674620167` succeeded; all 63 grid winners/189 starts reviewed, with broad low-SNR profiles supporting weak constraints. Stage-3c `gate-c-agn-shot-noise` compares paired background-only/source-shot-noise arms with identical oracle variance weights, using archived n=4 scenes. Practical weights, broader ensembles and PSF mismatch remain open. See `benchmarks/agn_nuclear_fraction/REVIEW.md`.
