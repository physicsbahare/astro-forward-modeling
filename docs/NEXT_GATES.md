...
- [ ] Zhuang & Shen PSF-mismatch AGN-host benchmark — actual C5d–C5i and C5k–C5q artifacts reviewed; C5j and C5m remain incomplete LOCAL diagnostics with preserved memory-cap failures. C5o matched-PSF run `33819349854` did not pass. C5p showed module/solver-dependent basins, and all four C5q DE-LHS seeds reached the unchanged cap without finite products. C5r now freezes the separate noiseless AGN/host=10 wrong-PSF free-shape diagnostic with the same starts, bounds, objective and cap; timeout and boundary results remain observables. See `benchmarks/zhuang_shen_2024/C5O_PROTOCOL.md` through `C5R_PROTOCOL.md`, `c5p_33823405733.json`, `c5q_33830661656.json` and `REVIEW.md`. Signed wings remain non-photon-ready. Review actual C5r artifacts before any noise, physical/chromatic PSF or survey-readiness step.
