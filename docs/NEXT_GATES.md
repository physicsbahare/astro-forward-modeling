...
- [ ] Zhuang & Shen PSF-mismatch AGN-host benchmark — actual C5d–C5i and C5k–C5n artifacts reviewed; C5j and C5m remain incomplete LOCAL diagnostics with preserved memory-cap failures. C5n `33806193712` and its repair rerun `33810187864` succeeded, but compact-corner sampling drift remains up to 0.273% and cross-code differences up to 1.940%; neither renderer is truth. C5o matched-PSF run `33819349854` had a split result: n=1 passed, while one n=4 high-contrast compact start hit the frozen process cap, so C5o did not pass. C5p separately freezes an Imfit LM/Nelder--Mead optimizer-path diagnostic without changing images, bounds or resources. See `benchmarks/zhuang_shen_2024/C5O_PROTOCOL.md`, `C5P_PROTOCOL.md`, `c5o_33819349854.json` and `REVIEW.md`. Signed wings remain non-photon-ready. Physical/chromatic PSF and survey-readiness remain open; do not start wrong-PSF free-shape fitting before C5p review.
