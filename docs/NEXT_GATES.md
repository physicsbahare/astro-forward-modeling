...
- [ ] AGN nuclear-fraction morphology contamination benchmark — Stage-0 anchor run `33632426495` confirmed successful; Stage-1 perfect-PSF/noiseless host-only versus Sérsic+PSF diagnostic implemented in `scripts/run_agn_nuclear_fraction_noiseless.py`, pending Actions results and scientific review. Noise and PSF mismatch remain separate. See `benchmarks/agn_nuclear_fraction/REVIEW.md`.
