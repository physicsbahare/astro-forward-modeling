...
- [ ] AGN nuclear-fraction morphology contamination benchmark — Stage-2b run `33656800320` succeeded; all 36 starts were reviewed without failures/bounds, and structural drift decreases with refinement. Stage-2c `gate-c-agn-galsim` adds a pinned independent host renderer and two FFT accuracy settings before noise. Independent convergence and full identifiability remain unproven. Noise and PSF mismatch remain separate. See `benchmarks/agn_nuclear_fraction/REVIEW.md`.
