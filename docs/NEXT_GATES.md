...
- [ ] AGN nuclear-fraction morphology contamination benchmark — Stage-2c run `33661985266` succeeded and independent-renderer artifacts were reviewed; numerical floors are measured, not universal acceptance bounds. Stage-3a `gate-c-agn-noise-pilot` freezes background-only host SNR=100/20/5, three paired realizations, preserving all starts and failures. Full identifiability, source-shot noise and PSF mismatch remain separate/open. See `benchmarks/agn_nuclear_fraction/REVIEW.md`.
