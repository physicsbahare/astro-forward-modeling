...
- [ ] Zhuang & Shen PSF-mismatch AGN-host benchmark — C5c run `33709361250` succeeded; all 6,816 evaluations/eight candidates reviewed. Independent search seeds agree with the C5b biased lower-n-bound solution; this is not physical recovery or global-optimum proof. C5d `gate-c-agn-empirical-psf-transfer` reuses pinned author F444W A/B PSFs and GalSim to verify effective-pixel transfer and fixed-shape flux bias before nonlinear empirical fits. Empirical/core-wing, chromatic and centroid questions remain open; see `benchmarks/zhuang_shen_2024/REVIEW.md` and `de_33709361250.json`.
